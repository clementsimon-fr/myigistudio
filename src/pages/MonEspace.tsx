  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [resReservations, resCards] = await Promise.all([
        supabase.from("reservations").select("*").eq("client_name", "Sophie").order("date", { ascending: false }),
        supabase.from("client_cards").select("*").eq("client_name", "Sophie").order("created_at", { ascending: false }),
      ]);
          ...
          <Tabs defaultValue="reservations">
            ...
            <TabsContent value="reservations" className="mt-4 space-y-3">
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <Link to="/calendrier"><Button>Réserver maintenant</Button></Link>
                </div>
              ) : (
                reservations.map((r) => (
            ...
            <TabsContent value="cartes" className="mt-4 space-y-4">
              {loading ? (
                ...
                <div className="text-center py-12">
                  <p className="text-muted-foreground">Aucune carte active.</p>
                </div>
              ) : (
                cards.map((card) => {
                    ...
                    <div key={card.id} className="rounded-xl border bg-card p-5">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-medium">
                          {card.used_sessions}/{card.total_sessions}
                        </span>
                      </div>
